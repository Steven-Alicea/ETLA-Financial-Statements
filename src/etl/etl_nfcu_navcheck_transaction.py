...
from src.etl.transform.transform_nfcu import correct_mismatch_rows, add_year_to_transaction_date_column, add_statement_period_to_dataframe, transform_navcheck_transaction_data
from src.etl.load.load import load_to_csv



column_names = ["Transaction Date", "Description", "Amount", "Fees", "Interest", "Principal", "Balance"]

target_directory = f"data/processed/NFCU"
target_file = "NFCU Navcheck Transactions.csv"


def etl_nfcu_navcheck_transactions(start_year, end_year):
    csv_files = []
    dataframes = []
    for i in range(start_year, end_year + 1):
        csv_directory = (f"data/raw/NFCU/Navcheck/{i}/Transactions")
    ...
    for file in csv_files:
        ...
        df = transform_navcheck_transaction_data(df)
...
def main():
    etl_nfcu_navcheck_transactions(2021, 2023)
