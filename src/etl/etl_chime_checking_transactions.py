...
from src.etl.transform.transform_chime import correct_mismatch_rows, add_statement_period_to_dataframe, transform_checking_transaction_data
from src.etl.load.load import load_to_csv



column_names = ["Transaction Date", "Description", "Type", "Amount", "Net Amount", "Settlement Date"]

target_directory = f"data/processed/Chime"
target_file = "Chime Checking Transactions.csv"


def etl_chime_checking_transactions(start_year, end_year):
    csv_files = []
    dataframes = []
    for i in range(start_year, end_year + 1):
        csv_directory = f"data/raw/Chime/Checking/{i}/Transactions"
    ...
    for file in csv_files:
        ...
        df = transform_checking_transaction_data(df)
...
def main():
    etl_chime_checking_transactions(2022, 2023)
