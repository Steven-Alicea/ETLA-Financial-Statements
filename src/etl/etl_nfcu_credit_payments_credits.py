...
from src.etl.transform.transform_nfcu import add_statement_period_to_dataframe, transform_credit_payments_credits
from src.etl.load.load import load_to_csv



column_names = ["Transaction Date", "Post Date", "Reference Number", "Description", "Submitted By", "Amount"]

target_directory = f"data/processed/NFCU"
target_file = "NFCU Credit Payments and Credits.csv"


def etl_nfcu_credit_payments_credits(start_year, end_year):
    csv_files = []
    dataframes = []
    for i in range(start_year, end_year + 1):
        csv_directory = (f"data/raw/NFCU/Credit/{i}/Payments And Credits")
    ...
    for file in csv_files:
        ...
        df = transform_credit_payments_credits(df)
...
def main():
    etl_nfcu_credit_payments_credits(2021, 2023)
